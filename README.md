...
Room II's isolated simplification is integrated: each of the five events uses an event title, one **Event paragraph**, quotation/source, then a separate **Kind of freedom** selector. Images remain optional. Earlier separate answers appear together in the paragraph box until it is edited; the original saved wording remains accessible under **Earlier saved writing (preserved)**. All museum, presentation and editor views use this same structure. No saved writing is migrated or deleted. Presentation pages omit empty responses and preserve the assignment's room and event order.
...
6. Use **Export Word document** for a dated, readable copy of every nonempty text field, including removed boards, legacy responses, captions and credits. It includes local drafts with a clear label and preserves a differing shared version alongside a conflict. Images are not embedded in the text export. The JSON backup remains the way to restore data into the app. For repeatable exports outside the browser, run `npm run export:writing -- /path/to/douglass-writing.docx`; it reads the current public shared exhibition and leaves it unchanged.
7. Use **Version history** to restore previous saves, or **Download backup** to keep a portable JSON copy. **Restore from backup** previews the changed fields before queuing them. Downloaded backups contain writing, not the class access key.
8. Check the **Presentation checklist**. Its completion counts check filled fields and word targets; students still need to verify quotation accuracy, analytical quality, chronological order and the overall argument.
