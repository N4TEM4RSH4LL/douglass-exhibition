...
function renderEntry() {
  const card = CARD_BY_ID[selected],
    ...
    next = CARDS[index + 1];
  $("#content").innerHTML =
    `<header class="room-intro"><span class="eyebrow">ROOM ${r.number} · EXHIBITION STUDIO</span><h1>${esc(r.title)}</h1><p>${esc(r.summary)}</p><div class="reading"><span>Reading</span>${esc(r.reading)}</div></header><article class="entry-card"><div class="entry-heading"><div><h2>${esc(card.label)}</h2><p>${esc(card.guide)}</p></div><span class="entry-progress" id="entry-progress"></span></div><form id="entry-form">${card.room === 2 && /^r2-stage-\d$/.test(card.id) ? `${fieldHTML(card, card.fields.find((f) => f.key === "analysis"))}<details class="previous-event-fields"><summary>Earlier detailed fields (optional)</summary>${card.fields.filter((f) => f.key !== "analysis").map((f) => fieldHTML(card, f)).join("")}</details>` : card.fields.map((f) => fieldHTML(card, f)).join("")}</form></article><div class="entry-bottom"><a href="./#room=${card.room}" target="_blank" rel="noopener">See Room ${r.number} in the museum ↗</a>${next ? `<button class="button" id="next-entry">Next: ${esc(next.label)} →</button>` : '<button class="button" id="review-all">Review the presentation →</button>'}</div>`;
