...
BEGIN
 ...
 IF FOUND THEN
  ...
  END IF;
  RETURN jsonb_build_object('ok',true,'field',jsonb_build_object('value',prior.value,'revision',prior.revision,'updatedAt',prior.changed_at,'writer',prior.writer));
 END IF;
 ...
 IF COALESCE(current_row.revision,0)<>p_expected THEN
  RETURN jsonb_build_object('ok',false,'current',jsonb_build_object('value',COALESCE(current_row.value,''),'revision',COALESCE(current_row.revision,0),'updatedAt',current_row.updated_at,'writer',COALESCE(current_row.writer,'')));
 END IF;
 ...
 RETURN jsonb_build_object('ok',true,'field',jsonb_build_object('value',current_row.value,'revision',current_row.revision,'updatedAt',current_row.updated_at,'writer',current_row.writer));
END; $$;
